...
// -----------------------------
if ($searchTerm !== '') {
    // Day 12, Module 2 Challenge: search across name, branch, AND course
    // (email search dropped in favor of course, to match today's spec).
    $sql = "SELECT * FROM students
            WHERE name LIKE ? OR branch LIKE ? OR course LIKE ?
            ORDER BY id DESC";
...
<div class="container pb-5">
  ...
  <form action="students.php" method="get" class="mb-4">
    <div class="input-group">
      ...
      <?php if ($searchTerm !== ''): ?>
        <a href="students.php" class="btn btn-outline-secondary"><i class="bi bi-x-circle"></i> Clear</a>
      <?php endif; ?>
    </div>
  </form>
        ...
        <tbody>
          <?php while ($row = mysqli_fetch_assoc($result)): ?>
            ...
            <tr class="<?php echo $rowClass; ?>">
              <td>
                <img src="<?php echo htmlspecialchars(getStudentPhoto($row['photo'])); ?>"
                     class="student-avatar" alt="<?php echo htmlspecialchars($row['name']); ?>" />
              </td>
              ...
              <td><?php echo $row['course'] ? highlight($row['course'], $searchTerm) : '—'; ?></td>
  ...
  <a href="index.html" class="btn btn-primary"><i class="bi bi-person-plus-fill"></i> Register New Student</a>
</div>
